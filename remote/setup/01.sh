...
# VARIABLES
# ==================================================================================== #
## echo "Timezone=$(timedatectl list-timezones | grep -i belgrade)" >> remote/setup/01.sh
TIMEZONE=Europe/Belgrade

# set the name of the new user to create
USERNAME=goplex

# Prompt to enter a password for the PostgreSQL goplex user (rather than hard-coding
# a password in this script).

read -p "Enter password for goplex DB user: " DB_PASSWORD 
echo "You have entered $DB_PASSWORD"
...
# Set up the goplex DB and create a user account with the password entered earlier.
sudo -i -u postgres psql -c "CREATE DATABASE goplex"
sudo -i -u postgres psql -d goplex -c "CREATE EXTENSION IF NOT EXISTS citext"
sudo -i -u postgres psql -d goplex -c "CREATE ROLE goplex WITH LOGIN PASSWORD '${DB_PASSWORD}'"

# Add a DSN for connecting to the goplex database to the system-wide environment
# variables in the /etc/environment file.
echo "GOPLEX_DB_DSN='postgres://goplex:${DB_PASSWORD}@localhost/goplex'" >> /etc/environment
